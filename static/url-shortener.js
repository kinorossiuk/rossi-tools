(() => {
  ...
  const shortUrl = (link) => link.short_url || `${window.location.origin}/s/${link.code}`;
  ...
  const showResult = (link, created) => {
    output.value = shortUrl(link); destination.textContent = `원본: ${link.url}`; result.hidden = false; output.focus(); output.select();
  ...
  const render = (links) => {
    ...
    links.forEach((link) => {
      const compact = shortUrl(link); const article = document.createElement('article'); article.className = 'url-shortener-item'; const main = document.createElement('div'); main.className = 'url-shortener-item-main';
      ...
      actions.append(makeButton('복사', 'ghost', async () => { try { await copyText(compact); setStatus('단축 URL을 클립보드에 복사했습니다.'); } catch (error) { setStatus(error instanceof Error ? error.message : '복사하지 못했습니다.', true); } }), makeButton('삭제', 'ghost url-shortener-delete', async (event) => {
        ...
        try { await request({ method: 'POST', body: JSON.stringify({ action: 'delete', id: link.id }) }); if (output.value === compact) result.hidden = true; setStatus('단축 URL을 삭제했습니다.'); await loadLinks(false); } catch (error) { button.disabled = false; setStatus(error instanceof Error ? error.message : '삭제하지 못했습니다.', true); }
      })); article.append(main, actions); items.append(article);
    });
  };
  const loadLinks = async (announce = true) => { reload.disabled = true; try { const payload = await request(); render(Array.isArray(payload.links) ? payload.links : []); if (!payload.bitly_configured) setStatus('Bitly API 토큰 설정이 필요합니다. 아래 안내의 cPanel 명령을 먼저 실행해 주세요.', true); else if (announce) setStatus('저장된 단축 URL 목록을 불러왔습니다.'); } catch (error) { items.replaceChildren(); count.textContent = '목록을 불러오지 못함'; setStatus(error instanceof Error ? error.message : '목록을 불러오지 못했습니다.', true); } finally { reload.disabled = false; } };
