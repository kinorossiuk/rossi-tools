...
<section class="tool-view url-shortener-tool" aria-labelledby="tool-title" data-url-shortener-tool data-csrf="<?= $urlShortenerCsrf ?>">
  ...
  <p class="tool-intro">긴 URL을 입력하면 <strong>is.gd/…</strong> 주소로 압축합니다. 생성된 주소는 바로 복사해 사용할 수 있습니다.</p>
  <section class="url-shortener-notice" aria-label="저장 및 공개 범위 안내"><div><strong>is.gd 서버 전송</strong><span>URL 발급을 위해 원본 URL을 is.gd API로 전송합니다. API 토큰이나 별도 설정은 필요하지 않습니다.</span></div><div><strong>민감 URL 차단</strong><span>토큰·비밀번호·세션·인증 코드 쿼리값이 든 URL은 압축할 수 없습니다. 단축 주소는 공개 공유용입니다.</span></div></section>
  ...
  <p class="url-shortener-status" id="url-shortener-status" role="status" aria-live="polite">URL을 입력하고 압축 버튼을 눌러 주세요. 별도 API 토큰 설정은 필요하지 않습니다.</p>
  <p class="url-shortener-help">is.gd의 서버 IP 기준 호출 한도가 적용됩니다. 통계 수집은 요청하지 않아 단축 발급 한도를 절약합니다.</p>
