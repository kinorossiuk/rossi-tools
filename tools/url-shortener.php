...
<section class="tool-view url-shortener-tool" aria-labelledby="tool-title" data-url-shortener-tool data-csrf="<?= $urlShortenerCsrf ?>">
  ...
  <p class="tool-intro">긴 URL을 입력하면 짧은 <strong>rossiuk.xyz/s/…</strong> 주소로 압축합니다. 생성된 주소는 바로 복사해 사용할 수 있습니다.</p>
  <section class="url-shortener-notice" aria-label="저장 및 공개 범위 안내"><div><strong>서버 저장</strong><span>단축 주소가 계속 동작하도록 원본 URL을 서버의 비공개 저장 공간에 보관합니다.</span></div><div><strong>단축 주소 공개</strong><span>주소를 아는 사람은 로그인 없이 원본 URL로 이동할 수 있습니다. 인증 토큰이 든 URL은 입력하지 마세요.</span></div></section>
  <form class="url-shortener-form" id="url-shortener-form" novalidate><label for="url-shortener-input">압축할 URL</label><div class="url-shortener-input-row"><input id="url-shortener-input" name="url" type="url" required maxlength="4096" inputmode="url" autocomplete="off" spellcheck="false" placeholder="https://example.com/very/long/url?with=query"><button class="primary" id="url-shortener-submit" type="submit">URL 압축</button></div><p class="url-shortener-help">http 또는 https 주소를 입력하세요. 프로토콜을 생략하면 https://를 자동으로 붙입니다.</p></form>
  ...
  <p class="url-shortener-status" id="url-shortener-status" role="status" aria-live="polite">URL을 입력하고 압축 버튼을 눌러 주세요.</p>
