...
- **URL 압축기**: 긴 URL을 is.gd의 `is.gd/…` 형식 짧은 주소로 만들고 복사·관리합니다.
...
`https://is.gd/XXXXXXX` 형식의 짧은 주소를 생성합니다.

- `http` 또는 `https` URL만 허용하며, 프로토콜을 생략하면 `https://`를 붙임
- is.gd API로 발급하며, API 토큰이나 별도 계정 설정은 필요 없고 생성·목록·삭제는 로그인한 사용자만 가능
- 원본 URL과 is.gd 링크 메타데이터는 웹 루트 밖 `~/.rossi-tools/short-links.json`에 저장
- `token`, `key`, `secret`, `password`, `session`, `code` 등 민감 쿼리값이 포함된 URL은 생성 차단
- 단축 주소는 공개 공유용이므로 개인정보·인증정보가 든 URL은 사용할 수 없음
- 발급은 서버 IP 기준 시간당 200개(일 환산 4,800개)까지이며, 통계 수집은 요청하지 않음
- is.gd에서 만든 주소는 목적지를 수정하거나 서비스에서 삭제할 수 없으며, 여기서 삭제하면 목록에서만 제거됨
