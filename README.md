...
PHP 웹호스팅에서 실행되며 QA 실무를 위해 직접 개발한 개인 업무 도구 플랫폼
...
- **URL 압축기**: 긴 URL을 `rossiuk.xyz/s/…` 형식 자체 주소로 만들고 복사·관리합니다.
...
- 생성 문자를 즉시 QR로 표시하고, 한 QR 용량을 넘는 긴 문자는 순서가 표시된 여러 QR로 자동 분할
...
- 외부 API를 호출하지 않으며, 생성·목록·삭제는 로그인한 사용자만 가능
- 원본 URL과 자체 단축 링크 메타데이터는 웹 루트 밖 `~/.rossi-tools/short-links.json`에 저장
- `token`, `key`, `secret`, `password`, `session`, `code` 등 민감 쿼리값이 포함된 URL은 생성 차단
- 단축 주소는 공개 공유용이므로 개인정보·인증정보가 든 URL은 사용할 수 없음
- 자체 도메인과 서버 저장 공간이 유지되는 동안 호출 횟수 제한 없이 사용할 수 있음
- 자체 주소는 여기서 삭제하면 더 이상 작동하지 않음

## 업무 링크 필터

대시보드의 업무 링크는 현재 브라우저에만 저장됩니다.
